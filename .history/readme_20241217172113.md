...
This is the main jist of the data that we will be using which shows the stats for each team. The NBA is now flooded with various Advanced Stats,but we will use the offensive and defensive rating from each season to determine how good or bad a team was on that side of the ball. Higher the Offensive rating the better a team was on offensive and the lower the defensive rating the better that team was at defending others. Both these categories are calculated with a formulat
